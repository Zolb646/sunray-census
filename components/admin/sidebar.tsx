      ...
      {adminNavItems.map((item) => {
        ...
        return (
          <Link
            ...
            onClick={onNavigate}
            className={cn(
              "group flex items-center justify-between rounded-[22px] px-4 py-3 text-sm transition-all",
              isActive
                ? "bg-white text-slate-900 shadow-[0_22px_44px_-26px_rgba(15,23,42,0.55)]"
...
export function Sidebar() {
  return (
    <aside className="hidden lg:sticky lg:top-4 lg:flex lg:h-[calc(100vh-2rem)] lg:w-72 lg:flex-col lg:self-start lg:px-4 lg:py-4">
      <div className="flex h-full flex-col rounded-[32px] border border-sidebar-border bg-sidebar px-4 py-5 text-sidebar-foreground shadow-[0_30px_80px_-38px_rgba(15,23,42,0.8)]">
        <Link
          href="/admin"
          className="flex items-center gap-3 rounded-[24px] border border-white/10 bg-white/6 px-4 py-4">
          ...
          <div>
            <p className="text-xl tracking-widest font-semibold">Sunray</p>
          </div>
        </Link>
